...
async function onMessage(env, message) {
    ...
    if (/^\/start(@\w+)?$/i.test(text)) {
        ...
        await sendMessageToUser(chatId, `${successMsg}\n你好，欢迎使用私聊机器人！`, { disable_web_page_preview: true });
...
async function onCallbackQuery(env, query) {
    ...
    // === A. 验证码处理逻辑 (已迁移到 Mini App + Turnstile，此处仅保留兼容处理) ===
    if (data.startsWith('verify_')) {
        // Mini App 验证不使用 callback 按钮，这里只是为了防止旧按钮被点击时出错
